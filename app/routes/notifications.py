...
async def save_fcm_token(request: FCMTokenRequest, user_id: str = Depends(get_user_id)):
    """
    ...
    return FCMTokenResponse(success=True, message=result["message"])
...
async def send_test_notification(
    ...
    result = await notification_service.send_notification_to_tokens(
        tokens=request.tokens, title=request.title, body=request.body, data=request.data
    )
...
async def remove_fcm_token(
    ...
    if not result["success"]:
        if (
            ...
            or "No FCM tokens found" in result["message"]
        ):
            raise HTTPException(
