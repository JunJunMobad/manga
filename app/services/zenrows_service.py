...
class ZenRowsService:
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    if hid and title:
                        print(
                            f"✅ ZenRows success! Got manga info for {manga_id}: {title} (hid: {hid})"
                        )
                        return {"hid": hid, "title": title, "full_data": data}
                    else:
                ...
                print(
                    f"❌ ZenRows error {response.status_code}: {response.text[:200]}..."
                )
                return None
    ...
    async def fetch_manga_chapters(self, manga_hid: str) -> Optional[Dict[str, Any]]:
        ...
        target_url = (
            f"https://api.comick.fun/comic/{manga_hid}/chapters?date-order=2&lang=en"
        )
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    print(
                        f"✅ ZenRows success! Fetched {len(chapters)} chapters for {manga_hid}"
                    )
                    return data
                except ValueError as e:
                    print(f"❌ ZenRows returned invalid JSON: {e}")
